...
class HuggingFaceModel(pl.LightningModule):
    ...
    def _load_element_class(self, element_config : DictConfig, possible_classes : Dict[str, Any]):
        if not element_config.name in possible_classes:
            raise ValueError(f"Unknown element '{element_config.name}'. Parsing is not implemented")
    ...
    def configure_optimizers(self):
        ...
        scheduler_class, scheduler_params = self._load_element_class(
            self.hyp_config.scheduler,
            schedulers
        )
        scheduler = scheduler_class(
            optimizer,
            T_max=self.hyp_config.epochs,
            **scheduler_params
        )
