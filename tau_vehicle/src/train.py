...
from pytorch_lightning.loggers import Logger, MLFlowLogger, CSVLogger
...
def check_if_already_active():
    ...
    if mlflow.active_run():
        print(f"Run with UUID {mlflow.active_run().info.run_id} is already active. Do you want to stop it? [Y/n].")
        if input().strip() in ['Y', 'y']:
            mlflow.end_run()
        else:
            print_abort("Aborting. You can resume this run with config parameter 'resume=True'.")
...
def load_model(config : DictConfig):
    if config.model.provider == 'huggingface':
        ...
        raise ValueError(f"Unknown model provider '{config.model.provider}'. Parsing is not implemented")
...
def init_trainer(
    ...
    use_gpu = torch.cuda.is_available() \
        and config.devices not in ('cpu', None, [])
