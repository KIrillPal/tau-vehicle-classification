...
class VehicleDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        if stage in ("fit", None):
            self.train_ds = VehicleDataset(
                self.config.data, "train", self.transforms["train"]
            )
            self.val_ds = VehicleDataset(
                self.config.data, "val", self.transforms["val"]
            )
            self.num_classes = len(self.train_ds.classes)

        if stage in ("test", None):
            self.test_ds = VehicleDataset(
                self.config.data, "test", self.transforms["test"]
            )
    ...
    def train_dataloader(self):
        return self._create_dataloader(
            self.train_ds, shuffle=self.config.data.loader.shuffle
        )
