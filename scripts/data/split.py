...
def split_dataset(
    ...
    ), "Tried to grab more than 100% data for test and val."
    ...
    for cls_dir in [d for d in input_dir.iterdir() if d.is_dir()]:
        ...
        train, temp = train_test_split(
            images, test_size=test_ratio + val_ratio, random_state=random_state
        )
        relative_ratio = test_ratio / (val_ratio + test_ratio)
        val, test = train_test_split(
            temp, test_size=relative_ratio, random_state=random_state
        )
